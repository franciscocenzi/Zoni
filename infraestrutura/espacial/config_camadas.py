...
CONFIG_CAMADAS_INTELIGENTE = {
    "lotes": {
        "nomes_prioritarios": [
            "Lotes", "Lote", "Lotes_Ativos", "Cadastro_Lotes",
            "parcelas", "parcela", "loteamento", "lots", "parcel"
        ],
        "tipo_geometria": "polygon",
        "filtro_atributos": ["matricula", "inscricao", "area", "area_m2", "área"],
        "prioridade": 12,
    },
    "zoneamento": {
...
def detectar_camada_inteligente(chave, projeto=None):
    ...
    def _norm(txt: str) -> str:
        if txt is None:
            return ""
        nf = unicodedata.normalize("NFKD", str(txt))
        nf = "".join(ch for ch in nf if unicodedata.category(ch) != "Mn")
        return nf.lower().strip()

    for camada in todas:
        pont = 0
        nome = _norm(camada.name())
        ...
        if camada.type() == QgsMapLayer.VectorLayer:
            ...
            # Atributos
            campos = [_norm(f.name()) for f in camada.fields()]
            for filtro in config.get("filtro_atributos", []):
                if _norm(filtro) in campos:
                    pont += 20
                    break
