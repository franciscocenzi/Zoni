...
def _montar_identificacao(dados: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mapa = {
        ...
        'tipo_logradouro': ['tipo_logradouro', 'tipo_lograd', 'tipo_logr', 'tipo_via', 'tipo_log', 'tipo', 'tl', 'tipolograd', 'tipo_de_logradouro', 'tipov'],
        'logradouro': ['logradouro', 'Logradouro', 'rua', 'Rua', 'nome_logradouro', 'nm_logradouro', 'logr', 'nome_rua', 'nome_via', 'endereco', 'Endereço'],
        'numero': ['número', 'numero', 'Número', 'num', 'nr', 'n_porta'],
        ...
        'area_m2': ['area_m2', 'area', 'Area_m2', 'área', 'Área', 'area_geom'],
    }
    ...
    # Concatenar tipo_logradouro se ele existir
    tipo = resultado.get('tipo_logradouro')
    logr = resultado.get('logradouro')
    if logr:
        s_logr = str(logr).strip()
        if tipo and isinstance(tipo, str) and str(tipo).strip() and str(tipo).lower() not in ["none", "null"]:
            s_tipo = str(tipo).strip()
            # Só concatena se o tipo já não estiver no início do logradouro
            if not s_logr.lower().startswith(s_tipo.lower()):
                resultado['logradouro'] = f"{s_tipo} {s_logr}"
        ...
        for k, v in dados.items():
            if 'area' in _normalizar_chave(k):
                try:
                    ...
                    QgsMessageLog.logMessage(f"Falha ao converter '{v}' para float (campo de area fallback): {e}", "Zôni v2", Qgis.Warning)
    return resultado
...
def _parametros_para_dict(params: Any) -> Dict[str, Any]:
    ...
    extras: Dict[str, Any] = {}
    d_extras = getattr(params, "extras", {})
    
    if isinstance(d_extras, dict):
        for key in ["RF", "RFU", "RF_Sec", "RL", "RLF", "HEMB", "Hemb_max", "AEMax", "vagas_min", "vagas",
                    "Lote_min", "UnAut_min", "Testad_min"]:
            if key in d_extras:
                extras[key] = d_extras[key]
            elif hasattr(params, key):
                extras[key] = getattr(params, key)
...
def construir_contexto_relatorio(
    ...
    # ------------------------------------------------------------------
    vias_detectadas = []
    if getattr(analise, "testadas", None) and getattr(analise.testadas, "testadas_por_logradouro", None):
        vias_detectadas = list(analise.testadas.testadas_por_logradouro.keys())

    def _tentar_melhorar_logradouro(ident: Dict[str, Any]):
        logr = ident.get("logradouro")
        # Se não tem logradouro no cadastro, mas a testada achou
        if not logr and vias_detectadas:
            ident["logradouro"] = vias_detectadas[0]
            return
            
        # Se tem logradouro no cadastro, tenta bater com a testada pra pegar o nome completo (Rua, Av...)
        if logr and vias_detectadas:
            s_logr = str(logr).strip().lower()
            for via in vias_detectadas:
                s_via = via.lower()
                # Verifica se parte do nome está um no outro para parear e absorver a versão oficial
                if s_logr in s_via or s_via in s_logr:
                    ident["logradouro"] = via
                    break

    if isinstance(dados_lote, list):
        ctx["identificacao"] = [_montar_identificacao(d) for d in dados_lote]
        for idx in ctx["identificacao"]:
            _tentar_melhorar_logradouro(idx)
    else:
        ctx["identificacao"] = _montar_identificacao(dados_lote)
        _tentar_melhorar_logradouro(ctx["identificacao"])
    ...
    # 2.1) Geometria de interseção detalhada (multi-zona)
    zg = getattr(analise, "zoneamento_geom", None)
    if zg is not None:
        try:
            ctx["zoneamento_geom"] = {
                "zonas": list(getattr(zg, "zonas", []) or []),
                "areas_por_zona": dict(getattr(zg, "areas_por_zona", {}) or {}),
                "area_total_m2": getattr(zg, "area_total_m2", None),
            }
        except Exception as e:
            QgsMessageLog.logMessage(f"Erro ao mapear area/zonas iterando objeto (zoneamento_geom): {e}", "Zôni v2", Qgis.Warning)
            ctx["zoneamento_geom"] = {}
    else:
        ctx["zoneamento_geom"] = {}
    ...
    if getattr(analise, "zoneamento_avaliacao", None) is not None:
        ...
        QgsMessageLog.logMessage(f"DEBUG -> Parametros_dict construído para indices legado: {parametros_dict}", "Zôni v2", Qgis.Info)
        ...
        # Se falhou lá no final, a gente pode extrair do zoneamento_resolvido pra salvar a tabela!
        zr_fallback = getattr(analise, "zona_resolvida", None)
        params_fallback = getattr(zr_fallback, "parametros", None) if zr_fallback else None
        
        QgsMessageLog.logMessage(f"DEBUG -> Avaliação de índices nula! Tentando puxar do resolvido: {params_fallback}", "Zôni v2", Qgis.Info)

        ctx["indices"] = {
            "zona": getattr(zr_fallback, "zona_principal", None) if zr_fallback else None,
            "conforme": None,
            "pendencias": [], "observacoes": [], "parametros": _parametros_para_dict(params_fallback),
        }
    ...
    if testadas is not None:
        try:
            ...
            QgsMessageLog.logMessage(f"Erro ao extrair e iterar testadas/confrontantes: {e}", "Zôni v2", Qgis.Warning)
