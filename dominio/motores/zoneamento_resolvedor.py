...
class ZoneamentoResolvedor:
    ...
    def resolver(
        ...
        zona_bruta = getattr(res_zon, "zona_principal", getattr(res_zon, "zona", None))
        if zona_bruta and zona_bruta not in zonas_incidentes:
            zonas_incidentes.append(zona_bruta)
            zonas_areas.setdefault(zona_bruta, 0.0)
        ...
        for za in zonas_aplicadas:
            param = self._parametros_por_zona.get(za.codigo.upper().replace(" ", ""))
            if param is not None:
                za.parametros = param
                # Anexa notas cadastradas no JSON à zona e à lista global
                notas_param = []
                for n in getattr(param, "notas", []):
                    try:
                        notas_param.append(str(int(n)))
                    except (TypeError, ValueError):
                        notas_param.append(str(n))

                for n in notas_param:
                    if n and n not in za.notas:
                        za.notas.append(n)
                    if n and n not in notas_ativas:
                        notas_ativas.append(n)
        ...
        if zona_principal_legado:
            parametros_legado = self._parametros_por_zona.get(zona_principal_legado.strip().upper().replace(" ", ""))
        ...
        return ZonaResolvida(
            ...
            macrozona=getattr(res_zon, "macrozona_principal", getattr(res_zon, "macrozona", None)),
            eixos=eixos,
            especiais=especiais,
            # LEGADO:
