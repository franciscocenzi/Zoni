...
def analisar_lote(
    ...
    servico_intersecao=None,
    servico_testadas=None,
    regras_risco=None,
    ...
    # ------------------------------------------------------------------
    if servico_intersecao:
        res_zon = servico_intersecao.intersecao_zoneamento(geom_lote)
        res_app = servico_intersecao.intersecao_app(geom_lote)
        res_risco = servico_intersecao.intersecao_risco(geom_lote)
    else:
    ...
    if camada_zoneamento is not None:
        try:
            if servico_intersecao:
                res_geom = servico_intersecao.calcular_zoneamento_incidente(
                    geom_lote, camada_zoneamento
                )
            else:
            ...
            QgsMessageLog.logMessage(f"Erro ao calcular zoneamento geométrico: {e}", "Zôni v2", Qgis.Critical)
            res_geom = None
    ...
    if camada_lotes is not None:
        try:
            if servico_testadas:
                res_testadas = servico_testadas.calcular(
                    geom_lote, camada_lotes, camada_logradouros, max_dist_testada_m
                )
            else:
            ...
            try:
                ...
                QgsMessageLog.logMessage(f"Falha ao registrar mensagem de erro de testadas no res_zon: {e2}", "Zôni v2", Qgis.Warning)
    ...
    # Flags finais aplicados no resolvedor
    nota10_flag = bool(nota10_ativada)
    nota37_flag = bool(nota37_ativada or detectou_frente_nota_37)
    ...
    if res_geom is not None:
        ...
        try:
            zona_resolvida = resolvedor.resolver(
                res_zon=res_zon,
                res_geom=res_geom,
                nota10_ativada=nota10_flag,
                nota37_ativada=nota37_flag,
            )

        except Exception as e:
            QgsMessageLog.logMessage(f"Erro ao resolver regras de sobreposição de zoneamento: {e}", "Zôni v2", Qgis.Critical)
            zona_resolvida = None
    ...
    if zona_resolvida is not None:
        ...
        if getattr(zona_resolvida, "notas_ativas", None):
            try:
                ...
                QgsMessageLog.logMessage(f"Falha ao registrar as notas do anexo III aplicadas: {e}", "Zôni v2", Qgis.Warning)
        ...
        if getattr(zona_resolvida, "motivo", None):
            try:
                ...
                QgsMessageLog.logMessage(f"Falha ao registrar o motivo/descrição da regra: {e}", "Zôni v2", Qgis.Warning)
        ...
        if (
            ...
            try:
                ...
                QgsMessageLog.logMessage(f"Falha ao registrar o tipo de regra: {e}", "Zôni v2", Qgis.Warning)
        ...
        if getattr(zona_resolvida, "zona_principal", None):
            try:
                ...
                QgsMessageLog.logMessage(f"Erro ao alinhar zona bruta à de referência, objeto imutável: {e}", "Zôni v2", Qgis.Warning)
    ...
    if zona_resolvida is not None and getattr(zona_resolvida, "zona_principal", None):
        ...
        if parametros is not None:
            ...
            try:
                ...
                QgsMessageLog.logMessage(f"Erro ao gravar falta de parâmetros da zona na lista geral: {e}", "Zôni v2", Qgis.Warning)
    else:
        ...
        try:
            ...
            QgsMessageLog.logMessage(f"Erro ao gravar fallback de zona de referência: {e}", "Zôni v2", Qgis.Warning)
    ...
    if camada_inclinacao is not None:
        ...
        try:
            if servico_intersecao:
                res_inclinacao = servico_intersecao.analisar_inclinacao(
                    geom_lote, camada_inclinacao, cenario.area_lote_m2
                )
            else:
            ...
            if res_inclinacao and hasattr(res_inclinacao, 'tem_app_por_inclinacao') and res_inclinacao.tem_app_por_inclinacao:
                try:
                    ...
                    QgsMessageLog.logMessage(f"Aviso ao registrar mensagem de APP de inclinação: {e}", "Zôni v2", Qgis.Warning)
            ...
            try:
                ...
                QgsMessageLog.logMessage(f"Aviso ao registrar erro de inclinação na lista: {e2}", "Zôni v2", Qgis.Warning)
            res_inclinacao = None
    else:
...
class MotorAnaliseLote:
    def __init__(
        ...
        self.utils_geometria = utils_geometria
        self.servico_intersecao = servico_intersecao
        self.servico_testadas = servico_testadas
    ...
    def analisar(
        ...
        geom_lote=None,
        cenario=None,
        caminho_parametros_zoneamento=None,
        nota10_ativada=False,
        nota37_ativada=False,
        max_dist_testada_m=DEFAULT_MAX_DIST_TESTADA_M,
        **kwargs,
    ):
        # Compatibilidade com chamadas que usam nomes alternativos/objeto de configuração
        if geom_lote is None:
            geom_lote = kwargs.pop("geometria_lote", None)
        if caminho_parametros_zoneamento is None:
            caminho_parametros_zoneamento = kwargs.pop("caminho_parametros", None)

        config = kwargs.pop("configuracao", None)
        if config is not None:
            nota10_ativada = getattr(config, "nota10_ativada", nota10_ativada)
            nota37_ativada = getattr(config, "nota37_ativada", nota37_ativada)
            max_dist_testada_m = getattr(
                config, "max_dist_testada_m", max_dist_testada_m
            )
            if caminho_parametros_zoneamento is None:
                caminho_parametros_zoneamento = getattr(
                    config, "caminho_parametros", None
                )

        if caminho_parametros_zoneamento is None:
            raise ValueError("caminho_parametros_zoneamento não informado")
